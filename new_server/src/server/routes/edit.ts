...
router.post('/profile', async (req: any, res: Response) => {
    ...
    const userIdClean = sanitizeInput(userId)
    const profileTextClean = sanitizeInput(profileText)
    const universityClean = sanitizeInput(university)
    const progressClean = sanitizeInput(progress)
